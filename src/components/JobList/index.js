...
class JobList extends React.Component {
  ...
  render() {
    ...
    return <div className={join('pa3 ', styles.jobList)}>
  ...
  renderJob(job, index) {
    return <div key={job.id || index}
      style={{ minWidth: '20rem' }} className={join("flex flex-column bg-white mw5 mw6-ns mr1 mb4 ba br3", styles.job)} >
      <div className={join("pa3 bb", styles.jobTitle)}>{job.title}</div>
      <div className={join("pa3 bb", styles.jobDescription)}>{job.description}</div>
