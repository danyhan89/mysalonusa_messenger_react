...
const Link = ({ children, onClick }) => {
  return <a href="" className={`${styles.link} db mb3`} onClick={(e) => {
    preventDefault(e);
    onClick();
  }} >{children}</a>
}
class Footer extends React.Component {
  ...
  render() {
    return <div className={`${styles.footer} f6 pa2 tl`}>
      <div >
        <Link onClick={() => {
  ...
  renderChooseLanguage() {
    ...
    return <Overlay closeable onClose={() => {
      this.setState({
        chooseLanguage: false
      })
    }}>
      <LanguagePopup onChange={lang => {
        let url = `/${lang}`
        if (this.props.state) {
