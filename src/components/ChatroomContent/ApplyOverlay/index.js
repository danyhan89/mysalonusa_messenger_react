...
class ApplyOverlay extends Component {
  ...
  render() {
    ...
    const actionIcon = this.state.sending ? <LoadingIcon size={32} /> : APPLY_ICON({ size: 32 })

    return <Overlay closeable onClose={onDismiss}>
      <div
        style={{ minWidth: '50vw' }}
        className={`br3`}
      >
        ...
        }} value={this.state.email} className={`f4 w-100`} />
        <div className="mt3">
        ...
        }} value={this.state.message} autoFocus={false} tag="textarea" rows={5} className={`f4 w-100`} />

        <ActionButton disabled={this.isSendDisabled()} className={`mt3 flex items-center`} onClick={() => this.onSend()}>
