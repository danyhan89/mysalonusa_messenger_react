...
class PaginationToolbar extends React.Component {
  ...
  render() {
    ...
    const items = [
      this.renderNavButton({ disabled: currentPage <= 1, onClick: () => this.gotoPage(1), children: <FirstIcon /> }),
      this.renderNavButton({ disabled: !hasPrevPage, onClick: () => this.gotoPrevPage(), children: <PrevIcon /> }),
      ...middlePages,

      this.renderNavButton({ disabled: !hasNextPage, onClick: () => this.gotoNextPage(), children: <NextIcon /> }),
      this.renderNavButton({ disabled: currentPage >= maxPage, onClick: () => this.gotoPage(maxPage), children: <LastIcon /> })
    ]

    return <div className={join("inline-flex flex-row br3", this.props.className, styles.pagination)} children={items}>

    </div>
  }

  renderNavButton({ disabled, onClick, children }) {
    return <button className={join(styles.navButton, styles.pageBlock, disabled && styles.disabled)} disabled={disabled} onClick={onClick}>{children}</button>
  }
  ...
  renderPage(children, page) {
    ...
    return <Tag className={join(styles.pageBlock, page != null && styles.page, 'pa2', isCurrent && styles.currentPage)} onClick={() => {
      if (page != null) {
        this.gotoPage(page)
      }
    }}>
