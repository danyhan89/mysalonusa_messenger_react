export default {
  welcome: "Vietnamese - Welcome to My Salon USA, {state}.",
  pleaseProvideNickname: "VO Please provide a nickname",
  whichCommunityPostJob: "VO - Which community do you want to post the job to?",
  whichStatePostJob: "VO - Which state do you want to post the job to?",
  userNameForPosting: "VO - What username do you want to post job?",
  emailForPosting: "VO - What email do you want to post job?",
  jobDescription: "VO - Job description",
  jobPost: "Job Post",
  comment: "Comment",
  report: "Report",
  bookmark: "Bookmark"
};
